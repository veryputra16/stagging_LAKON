...
class EmailsController extends Controller
{
    ...
    public function sendDiagnoEmail($request)
    {
        ...
        $send = $controller->laravelMail($address, $name, $subject, $data, [], [], null);
    ...
    public function validatingEmailSettingsUpdate($id, MailRequest $request)
    {
        try {
            ...
            return $this->validatingEmailSettings(new MailRequest($request->all()), $id);
        } catch (Exception $ex) {
            $message = $ex->getMessage();
            if (imap_last_error()) {
                $message = imap_last_error();
    ...
    public function checkImapStream($imap_stream)
    {
        if (is_resource($imap_stream)) {
            ...
            throw new \InvalidArgumentException('Invalid IMAP stream provided.');
        }
